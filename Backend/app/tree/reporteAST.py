...
import controller.AST as arbol
import os
...
class DOTAST: 
    ...
    def __init__(self):
        print("Generando el TextDot para el Arbol de navidad del AST")

    def getDot(self,instrucciones):
        global contador, TextDot, lista_instrucciones
        lista_instrucciones = instrucciones
        contador = 2
        TextDot = Digraph('AST', node_attr={'shape': 'box', 'style': 'filled','color': 'whitesmoke'}, edge_attr={'color':'green'})
        ...
        while tam < len(lista_instrucciones) :
            ...
            tam = tam +1
            print(TextDot)
        TextDot.render('reporte_AST', cleanup=True)
