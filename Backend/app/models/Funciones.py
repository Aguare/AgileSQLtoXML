...
def Between(exp1, exp2,ts):
    if isinstance(exp2, Operacion_Logica_Binaria):
        op1 = controller.AST.resolver_operacion(exp2.op1,ts)
        op2 = controller.AST.resolver_operacion(exp2.op2,ts)
...
def In(exp1, exp2, ts):
    lexp = []
    for exp in  exp2:
        op = controller.AST.resolver_operacion(exp,ts)
        lexp.append(op)
    if exp1 in lexp:
        return True
    else:
...
def Like(exp1,exp2,ts):
    if isinstance(exp2, (str,Operando_Cadena)):
        ...
        if isinstance(op, Operando_Cadena):
            op = controller.AST.resolver_operacion(op,ts)
        op = op.replace('%','')
        if op in exp1:
            return True
        else:
...
def Extract(medida, date_time,ts): # YYYY-MM-DD HH:MM:SS
    datos = controller.AST.resolver_operacion(date_time,ts)
    if len(datos) == 10:
...
def Date_Part(medida, date_time,ts): # W years X months Y days Z hours A minutes B seconds
    datos = controller.AST.resolver_operacion(date_time,ts)
    datos = datos.lower()
    datos_med = controller.AST.resolver_operacion(medida,ts)
...
def Greatest(exp,ts):
    lexp = []
    for exp1 in  exp:
        op = controller.AST.resolver_operacion(exp1,ts)
        lexp.append(op)
    return max(lexp)
